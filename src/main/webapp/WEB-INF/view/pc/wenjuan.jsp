...
<div class="layui-container layadmin-cmdlist-fluid">
    <h1 style="text-align: center;">校园论坛-问卷调查页面</h1>
    <div>
        <form id="ketiForm" action="${ctx}/pc/submitWenjuan" method="post">
            ...
            <c:if test="${score==null}">
                <c:forEach var="item" items="${questionnaireList}" varStatus="ss">
            ...
            <c:if test="${score!=null}">
                    <div class="layui-elem-quote" style="margin: 20px;">
                        <h2 style="color: red;">你已完成问卷调查！</h2>
                        <p style="font-size: 18px; margin-bottom: 15px;">您的问卷分数为：${score}</p>
                    </div>
            </c:if>
            <div class="layui-elem-quote" style="margin: 20px;">
                <c:if test="${sessionScope.loginUserinfo==null}">
                    <h2 style="color: red;">登录以后才能参加问卷调查考试</h2>
                </c:if>
                <c:if test="${sessionScope.loginUserinfo!=null&&(score==null||score==0)}">
