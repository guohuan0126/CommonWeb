layui.config({
    ...
    active = {
        search: function () {
            var keyword = $('#keyword')
            table.reload(  //执行重载
                'liuyanList',  //此处填写重载table.reload的id属性
                {
                    page: {
                        curr: 1     //重新从第 1 页开始
                    },
...
//数据表格
    table.render({
        id: 'liuyanList',
        elem: '#liuyanList'
        , url: ctx + '/manage/queryLiuyanList' //数据接口
        , cellMinWidth: 80
        , limit: 10//每页默认数
        , limits: [10, 20, 30, 40]
        , cols: [[ //表头
            ...
            {field: 'content', title: '留言内容', width: 140, align: 'center'},
    ...
    table.on('tool(test)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('真的删除行么?', function (index) {
                $.ajax({
                    url: ctx + '/manage/deleteLiuyan?id=' + data.id,
                    type: "get",
                    success: function (d) {
                        if (d.code == 0) {
                            table.reload('liuyanList', {})
                        } else {
            ...
            var url = ctx + "/manage/editLiuyan?id=" + data.id;//路径拼接
            location.href = url;
        } else if (obj.event === 'infoDetail') {
            var url = ctx + "/manage/liuyanInfo?id=" + data.id;
            layer.open({
                type: 2,
                btn: ['关闭'],
                btnAlign: 'c',
...
//批量删除留言
    $(".batchDel").click(function () {
        var checkStatus = table.checkStatus('liuyanList')
...
//调用删除接口
                $.ajax({
                    url: 'deletesLiuyan?idsStr=' + idsStr,//接口地址
                    type: "get",
                    success: function (d) {
                        if (d.code == 0) {
//重载表格
